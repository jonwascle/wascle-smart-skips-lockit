...
import { LogOut, Plus, MapPin, Users, Package, ClipboardList, Building2, Trash2, Pencil, X } from "lucide-react";
...
export default function AdminPanel({ operative, onLogout }) {
  ...
  const renameCustomer = async (id) => {
    if (!editingCustomerName.trim()) return;
    const { error } = await sb.from("customers").update({ name: editingCustomerName.trim() }).eq("id", id);
    if (error) { flash(error.message); return; }
    setEditingCustomerId(null);
    await loadAll();
    flash("Customer renamed.");
  };
  const deleteCustomer = async (c) => {
    const dependents = skips.filter((s) => s.customer_id === c.id).length + operatives.filter((o) => o.customer_id === c.id).length;
    if (dependents > 0) {
      flash(`Can't delete — this customer still has ${dependents} smart skip(s) or operative(s) linked to it. Remove those first.`);
      return;
    }
  ...
  const addOperative = async () => {
    ...
    try {
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/create-operative`, {
        ...
        body: JSON.stringify({ action: "create", name: newOpName.trim(), email: newOpEmail.trim(), customerId: newOpCustomerId, departmentId: newOpDepartmentId || null }),
      });
  ...
  const [editOpCustomerId, setEditOpCustomerId] = useState("");
  const [editOpDepartmentId, setEditOpDepartmentId] = useState("");

  const startEditOperative = (o) => {
    setEditingOpId(o.id);
    setEditOpName(o.name);
    setEditOpCustomerId(o.customer_id || "");
    setEditOpDepartmentId(o.department_id || "");
  };
  ...
  const saveEditOperative = async () => {
    ...
    const res = await fetch(`${SUPABASE_URL}/functions/v1/create-operative`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${session.access_token}` },
      body: JSON.stringify({ action: "update", operativeId: editingOpId, name: editOpName.trim(), customerId: editOpCustomerId, departmentId: editOpDepartmentId }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.error) { flash(data.error || "Something went wrong."); return; }
    setEditingOpId(null);
  ...
  const deleteOperative = async (o) => {
    ...
    const res = await fetch(`${SUPABASE_URL}/functions/v1/create-operative`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${session.access_token}` },
      body: JSON.stringify({ action: "delete", operativeId: o.id }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.error) { flash(data.error || "Something went wrong."); return; }
    await loadAll();
                ...
                {editingCustomerId === c.id ? (
                  <div className="flex gap-2">
                    <input value={editingCustomerName} onChange={(e) => setEditingCustomerName(e.target.value)}
                      className="flex-1 text-sm rounded-md py-2 px-3" style={inputStyle}
                      onKeyDown={(e) => { if (e.key === "Enter") renameCustomer(c.id); }} autoFocus />
                    <button onClick={() => renameCustomer(c.id)} className="px-3 rounded-md text-sm" style={{ background: AMBER, color: CHARCOAL, fontFamily: bodyFont }}>Save</button>
                    <button onClick={() => setEditingCustomerId(null)} className="px-3 rounded-md text-sm" style={{ background: PAPER, border: `1px solid ${LINE}`, color: STEEL, fontFamily: bodyFont }}><X size={14} /></button>
                  </div>
                ) : (
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => { setEditingCustomerId(c.id); setEditingCustomerName(c.name); }} className="p-1.5 rounded-md" style={{ color: STEEL }}><Pencil size={14} /></button>
              ...
              <Card key={o.id}>
                {editingOpId === o.id ? (
                  <div className="flex flex-col gap-2">
                    <input value={editOpName} onChange={(e) => setEditOpName(e.target.value)} className="text-sm rounded-md py-2 px-3" style={inputStyle} />
                    <select value={editOpCustomerId} onChange={(e) => { setEditOpCustomerId(e.target.value); setEditOpDepartmentId(""); }} className="text-sm rounded-md py-2 px-3" style={inputStyle}>
                      <option value="">No customer</option>
                      {customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                    <select value={editOpDepartmentId} onChange={(e) => setEditOpDepartmentId(e.target.value)} disabled={!editOpCustomerId} className="text-sm rounded-md py-2 px-3" style={inputStyle}>
                      <option value="">No department</option>
                      {departments.filter((d) => d.customer_id === editOpCustomerId).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
                    </select>
                    <div className="flex gap-2">
                      <button onClick={saveEditOperative} className="flex-1 py-2 rounded-md text-sm" style={{ background: AMBER, color: CHARCOAL, fontFamily: bodyFont }}>Save</button>
                      <button onClick={() => setEditingOpId(null)} className="px-3 rounded-md text-sm" style={{ background: PAPER, border: `1px solid ${LINE}`, color: STEEL, fontFamily: bodyFont }}><X size={14} /></button>
                    </div>
                  </div>
                ) : (
                  <div className="flex justify-between items-start">
